...
\begin{itemize}
	\item \textbf{Web Server Spring}: questa componente è in esecuzione su una istanza di Azure Spring Cloud e accessibile dall'applicazione client;
	\item \textbf{Database MySQL}: istanza di Azure MySQL;
...
Il coordinatore potrà invece scegliere dei servizi diversi da quelli di un utente volontario (figura \ref{fig:qtApp2}a). Infatti, potrà inserire nuovi volontari (figura \ref{fig:qtApp2}b), creare nuove squadre e cancellare degli utenti (figura \ref{fig:qtApp2}c).

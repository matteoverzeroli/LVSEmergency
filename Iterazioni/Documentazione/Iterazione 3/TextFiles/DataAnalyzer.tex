...
\begin{algorithm}
	...
	\For{$i = 1$ \KwTo $num\_areas$}{
		\textbf{badWeatherAllertsCreator}($area\_DS$ [$i$ ; `nameAprStation'], $area\_DS$[$i$ ; `idArea'], $i$, $old\_final\_time\_list$, $old\_delta\_list$)\;
		\textbf{fogFrostAllertsCreator}($area\_DS$ [$i$ ; `nameAprStation'], $area\_DS$[$i$ ; `idArea'], $i$, $old\_final\_time\_list$)\;
		}
...
\begin{algorithm}
	...
	\Fn{badWeatherAllertsCreator(station\_code, areaID, index, old\_final\_time\_list, old\_delta\_list)}{
		\textbf{/* Inizializzazione delle variabili */}\;
...
\begin{algorithm}
	...
	\Fn{fogFrostAllertsCreator(station\_code, areaID, index, old\_final\_time\_list)}{
		...
		\textbf{tempURForcaster(`temperature', \textit{summaryT})}\;
		\textbf{tempURForcaster(`umidity', \textit{summaryUR})}\;
	}
\end{algorithm}
...
\begin{algorithm}
	...
	\Fn{tempURForcaster(parameter, dataToUpdate)}{
		\textbf{return}\;
	}
\end{algorithm}

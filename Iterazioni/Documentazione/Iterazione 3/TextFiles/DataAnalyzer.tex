...
\begin{algorithm}
	...
	\For{$i = 1$ \KwTo $num\_areas$}{
		...
		$t \gets$ \textbf{FogFrostAllertsCreator.init}($area\_DS$ [$i$ ; `nameAprStation'], $area\_DS$[$i$ ; `idArea'], $conn1$, $cursor1$, $i$, $old\_final\_time\_list$)\;
		$w \gets$ \textbf{BadWeatherAllertsCreator.init}($area\_DS$ [$i$ ; `nameAprStation'], $area\_DS$[$i$ ; `idArea'], $conn2$, $cursor2$, $i$, $old\_final\_time\_list$, $old\_delta\_list$)\;
		$fogFrostAllertsCreators$.append($t$)\;
		$badWeatherAllertsCreators$.append($w$)\;
		}
	\For{$i = 1$ \KwTo $num\_areas$}{
		\textbf{\textit{fogFrostAllertsCreators}[\textit{i}].run()}\;
		\textbf{\textit{badWeatherAllertsCreators}[\textit{i}].run()}\;
		}
	\For{$i = 1$ \KwTo $num\_areas$}{
		\textbf{\textit{fogFrostAllertsCreators}[\textit{i}].join()}\;
		\textbf{\textit{badWeatherAllertsCreators}[\textit{i}].join()}\;
		}
...
\begin{algorithm}
	\SetKwProg{Fn}{Def}{ is}{end}
	\SetKwProg{Class}{Class}{:}{end}
	\SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{end}
	\caption{generatore di allerte maltempo, parte 1}
	\Class{badWeatherAllertsCreator as Thread}{
		\Fn{init(station\_code, areaID, conn, cursor, index, old\_final\_time\_list, old\_delta\_list)}{
			\textbf{this}.$station\_code \gets station\_code$\;
			\textbf{this}.$areaID \gets areaID$\;
			\textbf{this}.$conn \gets conn$\;
			\textbf{this}.$cursor \gets cursor$\;
			\textbf{this}.$index \gets index$\;
			\textbf{this}.$old\_final\_time\_list \gets old\_final\_time\_list$\;
			\textbf{this}.$old\_delta\_list \gets old\_delta\_list$\;
			$recent\_time \gets$ inizializza con una data e un orario passati\;
		}
		\Fn{join()}{
			metodo ereditato dalla superclasse \textit{Thread}\;
		}
	}
\end{algorithm}
...
\begin{algorithm}
	...
	\SetKwProg{Class}{Class}{:}{end}
	\SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{end}
	\caption{generatore di allerte maltempo, parte 2}
	\Class{badWeatherAllertsCreator as Thread}{
		\Fn{run()}{
			$cursor$.executeQuery(SELECT max(time) FROM aprsdata WHERE name = station\_code)\;
			$recent\_time \gets$ $cursor$.fetchAll()\;
			\If{$recent\_time = old\_final\_time\_list\left[i\right]$}{
				chiudi la connessione $conn$\;
				chiudi il cursore $cursor$\;
				\textbf{return}\;
			}
			$cursor$.executeQuery(SELECT time, pressure FROM aprsdata WHERE name = station\_code)\;
			$info \gets cursor$.fetchAll()\;
			...
			$num\_obs\_24h \gets$ length($pressure\_DS$)\;
			$p\_mean \gets$ mean($pressure\_DS$[all ; `pressure'])\;
			\For{$i = 1$ \KwTo $num\_obs\_24h$}{
				$pressure\_DS$[$i$; `pressure'] $\gets pressure\_DS$[$i$ ; `pressure'] - $p\_mean$\; 
			}
			selezione in $pressure\_DS$ delle osservazioni relative soltanto alle ultime \num{3} ore\;
			...
			\uIf{$delta \ge 0$}{
				...
				inserisci l'allerta nel DB tramite $cursor$.executeQuery()\;
			}
			...
			fai il commit delle modifiche al DB\;
			chiudi la connessione $conn$\;
			chiudi il cursore $cursor$\;
		}	
	}
...
\begin{algorithm}
	...
	\Class{fogFrostAllertsCreator as Thread}{
		\Fn{init(station\_code, areaID, conn, cursor, index, old\_final\_time\_list}{
			\textbf{this}.$station\_code \gets station\_code$\;
			\textbf{this}.$areaID \gets areaID$\;
			\textbf{this}.$conn \gets conn$\;
			\textbf{this}.$cursor \gets cursor$\;

...
\begin{algorithm}
	...
	\Fn{badWeatherAllertsCreator(station\_code, areaID, index, old\_final\_time\_list, old\_delta\_list)}{
		...
		$pressure\_DS\left[ \right]\gets$ recupera i dati di pressione atmosferica delle ultime \num{24} ore della stazione $station\_code$\;
...
\begin{algorithm}
	...
	\Fn{fogFrostAllertsCreator(station\_code, areaID, index, old\_final\_time\_list)}{
		...
		\textcolor{red}{ATTENZIONE DA RIVEDERE CODICE}
		\For{$j=1$ \KwTo $num\_columns$}{
			\For{$i=1$ \KwTo $num\_rows$}{
				$summaryTd$[$i$ ; $j$] $\gets$ calcolo temperatura di rugiada con l'approssimazione di Magnus-Tetens;
			}
		}
		\For{$i=1$ \KwTo $num\_rows$}{
...
\begin{algorithm}
	...
	\textcolor{red}{sistemare intervalli e scritte query}
